...
DEFAULT_BASE_COLOR_BRICH = [235, 225, 215]
...
def gaussian_noise(height: int, width: int) -> Image.Image:
    ...
    cv2.randn(image, DEFAULT_GAUSSIAN_NOISE_MEAN, DEFAULT_GAUSSIAN_NOISE_STD)
    ...
    for _ in range(strain_count):
        ...
        if actual_size_y > 0 and actual_size_x > 0:
            ...
            image[y:end_y, x:end_x, :] = np.clip(
                image[y:end_y, x:end_x, :] - (darkness * strain_region[..., None]), 
                0, 255
            )
...
def brich(
    ...
    """
    Generate a textured 'brich' background by stamping many small irregular circular spots.
    ...
    spots = int(params.get("spots", max(1, int(150 * texture))))
    ...
    base_color = np.array(DEFAULT_BASE_COLOR_BRICH, dtype=np.float32)
    ...
    for _ in range(spots):
        ...
        pixel_noise = np.random.uniform(
            1.0 - irregularity, 1.0 + irregularity, 
...
def parchment(
    ...
    base_color = np.array(
        params.get("base_color", DEFAULT_BASE_COLOR_PARCHMENT), dtype=np.float32
    )
...
def image(height: int, width: int, image_dir: str) -> Image.Image:
    ...
    if len(images) > 0:
        ...
        pic = Image.open(os.path.join(image_dir, selected_image))
        ...
        if pic.size[0] == width:
            ...
            crop_x = np.random.randint(0, pic.size[0] - width)
            
        if pic.size[1] == height:
            crop_y = 0
        else:
            crop_y = np.random.randint(0, pic.size[1] - height)
