...
def generate_perlin_noise(
    ...
    noise_map = np.vectorize(
        lambda i, j: pnoise2(i * scale, j * scale, octaves=octaves)
    )(y, x)
    
    return noise_map

def simulate_paper_fiber_texture(
    width: int, 
    height: int, 
    fiber_density: float = 0.2
) -> np.ndarray:
    """
    ...
    try:
        ...
        return np.random.randint(
            0, int(255 * fiber_density), (height, width, 3), dtype=np.uint8
        )


def simulate_fold_crease(
    image: np.ndarray, 
    fold_lines: List[Tuple[int, int, int, int]], 
    fold_intensity: float = 0.5
) -> np.ndarray:
    """
    ...
    for x1, y1, x2, y2 in fold_lines:
        ...
        shadow_mask = (
            (y_coords - y1) * (x2 - x1) - (x_coords - x1) * (y2 - y1) > 0
        )
        ...
        highlight_effect = (
            fold_profile * (~shadow_mask) * fold_intensity * HIGHLIGHT_INTENSITY
        )
...
def simulate_ink_bleed(
    ...
    bleed_effect = cv2.GaussianBlur(
        bleeding_mask.astype(np.float32), (kernel_size, kernel_size), 0
    )
...
def apply_perspective_distortion(
    ...
    dst = src + np.random.randint(
        -corner_displacement, corner_displacement + 1, size=(4, 2)
    )
...
def simulate_scanner_artifacts(
    image: np.ndarray, 
    ...
    result[ys, :, :] = np.clip(
        result[ys, :, :] - line_intensities[:, None, None], 0, 255
    )
